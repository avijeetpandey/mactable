...
//  mactable
//
//  Pure-Swift MD5. Used only for legacy Postgres MD5 password auth.
//

import Foundation

enum MD5 {
    static func hexHash(_ message: Data) -> String {
    ...
    static func digest(_ message: Data) -> [UInt8] {
        ...
        for i in 0..<8 { bytes.append(UInt8((originalLength >> (8 * i)) & 0xFF)) }
        ...
        for chunkStart in stride(from: 0, to: bytes.count, by: 64) {
            ...
            for j in 0..<16 {
                let off = chunkStart + j * 4
                M[j]  = UInt32(bytes[off])
                M[j] |= UInt32(bytes[off+1]) << 8
                M[j] |= UInt32(bytes[off+2]) << 16
                M[j] |= UInt32(bytes[off+3]) << 24
            }
